...
=end
  ...
  def self.all_terms_full_graph(limited_terms=nil)
    ...
    if limited_terms.nil?
      all_terms = HomosaurusV2.find_with_conditions("*:*", rows: '10000', fl: 'id,identifier_ssi,prefLabel_tesim, altLabel_tesim, description_tesim, issued_dtsi, modified_dtsi, exactMatch_tesim, closeMatch_tesim, broader_ssim, narrower_ssim, related_ssim, topConcept_ssim' )
    ...
    all_terms.each do |current_term|
      base_uri = ::RDF::URI.new("http://homosaurus.org/terms/v2/#{current_term['identifier_ssi']}")
      ...
      if current_term['broader_ssim'].present?
        current_term['broader_ssim'].each do |current_broader|
          graph << [base_uri, ::RDF::Vocab::SKOS.broader, ::RDF::URI.new("http://homosaurus.org/terms/v2/#{current_broader.split('/').last}")] if current_broader.present?
        end
      end

      if current_term['narrower_ssim'].present?
        current_term['narrower_ssim'].each do |current_narrower|
          graph << [base_uri, ::RDF::Vocab::SKOS.narrower, ::RDF::URI.new("http://homosaurus.org/terms/v2/#{current_narrower.split('/').last}")] if current_narrower.present?
        end
      end

      if current_term['related_ssim'].present?
        current_term['related_ssim'].each do |current_related|
          graph << [base_uri, ::RDF::Vocab::SKOS.related, ::RDF::URI.new("http://homosaurus.org/terms/v2/#{current_related.split('/').last}")] if current_related.present?
        end
      end
      ...
      if current_term['topConcept_ssim'].present?
        current_term['topConcept_ssim'].each do |top_concept|
          graph << [base_uri, ::RDF::Vocab::SKOS.hasTopConcept, ::RDF::URI.new("http://homosaurus.org/terms/v2/#{top_concept}")] if top_concept.present?
        end
      end
  ...
  def full_graph
    base_uri = ::RDF::URI.new("http://homosaurus.org/terms/v2/#{self.identifier}")
    ...
    self.broader.each do |current_broader|
        graph << [base_uri, ::RDF::Vocab::SKOS.broader, ::RDF::URI.new("http://homosaurus.org/terms/v2/#{current_broader.id.split('/').last}")] if current_broader.present?
    end

    @broadest_terms = []
    get_broadest self
    @broadest_terms.each do |broad_term|
      graph << [base_uri, ::RDF::Vocab::SKOS.hasTopConcept, ::RDF::URI.new("http://homosaurus.org/terms/v2/#{broad_term}")]
    end

    self.narrower.each do |current_narrower|
      graph << [base_uri, ::RDF::Vocab::SKOS.narrower, ::RDF::URI.new("http://homosaurus.org/terms/v2/#{current_narrower.id.split('/').last}")] if current_narrower.present?
    end

    self.related.each do |current_related|
      graph << [base_uri, ::RDF::Vocab::SKOS.related, ::RDF::URI.new("http://homosaurus.org/terms/v2/#{current_related.id.split('/').last}")] if current_related.present?
    end
